...
export default function QuizGame({ questions, timeLimit, showAnswers, onFinish }: Props) {
  ...
  const handleSelect = useCallback(
    (optIdx: number) => {
      ...
      setAnswered(true);
      const answer: PlayerAnswer = { question_id: question.id, selected_option: optIdx, is_correct: isCorrect, time_spent: timeSpent };
  ...
  const goNext = useCallback(
    (currentAnswers?: PlayerAnswer[]) => {
      ...
      setAnimState("out");
      setTimeout(() => { setCurrentIdx((i) => i + 1); setSelectedOption(null); setAnswered(false); }, 200);
    },
  ...
  const getOptionStyle = (idx: number) => {
    ...
    if (!answered) return `border-border hover:${color.border} hover:${color.bg} hover:shadow-md hover:-translate-y-0.5 cursor-pointer`;
    if (showAnswers) {
      if (idx === correctIdx) return "border-emerald-500 bg-emerald-500/10";
      if (idx === selectedOption && !question.options[idx].is_correct) return "border-red-500 bg-red-500/10";
    ...
    <div className="min-h-screen pt-20 pb-12 px-4">
      <div className="max-w-[700px] mx-auto">
        <div className="flex items-center gap-3 mb-6">
          <Progress value={((currentIdx + (answered ? 1 : 0)) / questions.length) * 100} className="flex-1 h-2" />
          <span className="text-sm font-medium text-muted-foreground whitespace-nowrap">
            {currentIdx + (answered ? 1 : 0)}/{questions.length}
          </span>
          {timeLimit && <TimerCircle timeLeft={timeLeft} totalTime={timeLimit} />}
        </div>

        <div key={currentIdx} className={animState === "in" ? "animate-slide-in-right" : "animate-slide-out-left"}>
          <p className="text-sm text-muted-foreground mb-2">
            {t("game.question")} {currentIdx + 1} {t("game.questionOf")} {questions.length}
          </p>
          <h2 className="text-xl font-medium mb-6 leading-relaxed" style={{ fontFamily: "'Space Grotesk', sans-serif" }}>
          ...
          {question.question_type === "true_false" ? (
            <div className="grid grid-cols-2 gap-3">
              {question.options.map((opt, i) => (
                <button key={i} disabled={answered} onClick={() => handleSelect(i)} className={`p-5 rounded-xl border-2 text-center font-medium text-lg transition-all duration-200 ${getOptionStyle(i)}`}>
                  <div className="flex items-center justify-center gap-2">
                    {opt.text === "True" ? <Check className="h-5 w-5" /> : <X className="h-5 w-5" />}
                    {opt.text}
                    {answered && showAnswers && i === correctIdx && <Check className="h-5 w-5 text-emerald-500" />}
                    {answered && showAnswers && i === selectedOption && !opt.is_correct && <X className="h-5 w-5 text-red-500" />}
                  </div>
                </button>
              ))}
            </div>
            ...
            <div className="space-y-3">
              {question.options.map((opt, i) => {
                ...
                return (
                  <button key={i} disabled={answered} onClick={() => handleSelect(i)} className={`w-full flex items-center gap-4 p-4 rounded-xl border-2 text-left transition-all duration-200 ${getOptionStyle(i)}`}>
                    <span className={`w-9 h-9 rounded-full flex items-center justify-center text-sm font-bold text-white shrink-0 ${color.circle}`}>
                      {answered && showAnswers && i === correctIdx ? <Check className="h-4 w-4" /> : answered && showAnswers && i === selectedOption && !opt.is_correct ? <X className="h-4 w-4" /> : LABELS[i]}
                    </span>
          ...
          {answered && showAnswers && (
            <div className="mt-6 flex justify-end animate-fade-in">
              <Button onClick={() => goNext()} className="gap-2 rounded-full px-6" size="lg">
                {isLast ? (<><Trophy className="h-5 w-5" /> {t("game.seeResults")}</>) : (<>{t("game.nextQuestion")} <ArrowRight className="h-4 w-4" /></>)}
              </Button>
            </div>
          )}
        </div>
