        ...
        <div className="flex items-center">
          ...
          <AccordionTrigger className="flex-1 py-3 hover:no-underline">
            <span className="text-sm font-medium">
              Q{index + 1}. {q.question_text || <span className="text-muted-foreground italic">Untitled question</span>}
            </span>
          </AccordionTrigger>
        </div>
          ...
          <div>
            ...
            >
              <div className="flex items-center gap-2">
                ...
                <Label htmlFor={`mc-${q.id}`} className="cursor-pointer">Multiple Choice</Label>
              </div>
              <div className="flex items-center gap-2">
                <RadioGroupItem value="true_false" id={`tf-${q.id}`} />
                <Label htmlFor={`tf-${q.id}`} className="cursor-pointer">True/False</Label>
              </div>
            </RadioGroup>
          </div>
...
export default function StepQuestions({ questions, setQuestions, onBack, onNext }: Props) {
  ...
  const duplicateQuestion = (q: Question) => {
    const dup: Question = { ...q, id: crypto.randomUUID(), order_index: questions.length };
  ...
  const handleNext = () => {
    if (questions.length < 3) { toast.error("Add at least 3 questions"); return; }
    for (let i = 0; i < questions.length; i++) {
      const q = questions[i];
      if (!q.question_text.trim()) { toast.error(`Question ${i + 1} needs text`); return; }
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">{questions.length} question{questions.length !== 1 ? "s" : ""} added (min 3)</p>
        </div>
      ...
      {questions.length > 0 && (
        <Progress value={Math.min((questions.length / 3) * 100, 100)} className="h-1.5" />
      )}
      ...
      {questions.length === 0 && (
        <div className="text-center py-16 text-muted-foreground">
          <p className="mb-3">No questions yet. Click "Add Question" to get started!</p>
        </div>
      )}
      ...
      <div className="flex justify-between pt-4">
        ...
        <Button onClick={handleNext} className="gap-2 rounded-full px-6">
          Preview & Publish <ArrowRight className="h-4 w-4" />
        </Button>
      </div>
    </div>
  );
