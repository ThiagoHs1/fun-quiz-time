...
export default function MyQuizzes() {
    ...
    if (name) {
      setPublished(
        getPublishedQuizzes().filter(
          (q) => q.creator_name.toLowerCase() === name.toLowerCase()
        )
      );
    }
  };
  ...
  const handlePublishDraft = (draft: DraftQuiz) => {
    ...
    toast.success(`Published! Share code: ${quiz.share_code}`);
    refresh();
  };
  ...
  return (
    <div className="min-h-screen pt-24 pb-20">
      <div className="container mx-auto px-4 max-w-3xl">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Creating as <span className="font-medium text-foreground">{creatorName}</span>
            </p>
          </div>
          <Link to="/create">
        ...
        {isEmpty && (
          <div className="text-center py-20">
            ...
            <Link to="/create">
              <Button className="rounded-full px-8">Create Your First Quiz</Button>
            </Link>
          </div>
        )}
        ...
        {drafts.length > 0 && (
          <div className="mb-8">
            <h2 className="text-sm font-semibold text-muted-foreground uppercase tracking-wider mb-3">Drafts</h2>
            <div className="space-y-3">
              {drafts.map((draft) => (
                <Card key={draft.id} className="p-0 overflow-hidden">
                  <div className="flex items-stretch">
                    <div
                      className="w-20 sm:w-28 shrink-0 flex items-center justify-center"
                      style={{ background: draft.formData.cover_gradient || "hsl(var(--muted))" }}
                    >
                        ...
                        <div className="min-w-0">
                          <div className="flex items-center gap-2 mb-1">
                            <h3 className="font-medium truncate">
                              {draft.formData.title || "Untitled Quiz"}
                            </h3>
                          ...
                          <div className="flex items-center gap-2 mt-2 flex-wrap">
                            {draft.formData.category && (
                              <Badge variant="outline" className={`text-xs ${categoryColors[draft.formData.category] || ""}`}>
                                {draft.formData.category}
                              </Badge>
                            )}
                            <Badge variant="outline" className={`text-xs capitalize ${difficultyColors[draft.formData.difficulty] || ""}`}>
                              {draft.formData.difficulty}
                            </Badge>
                    ...
                    <div className="flex flex-col justify-center gap-1 p-3 border-l border-border">
                      <Button
                        variant="ghost" size="sm" className="gap-1.5 justify-start"
                      ...
                      <Button
                        variant="ghost" size="sm" className="gap-1.5 justify-start text-destructive hover:text-destructive"
                        onClick={() => setDeleteTarget({ id: draft.id, title: draft.formData.title || "Untitled Quiz", type: "draft" })}
                      >
        ...
        {published.length > 0 && (
          <div>
            <h2 className="text-sm font-semibold text-muted-foreground uppercase tracking-wider mb-3">Published</h2>
            <div className="space-y-3">
              {published.map((quiz) => (
                <Card key={quiz.id} className="p-0 overflow-hidden">
                  <div className="flex items-stretch">
                    <div
                      className="w-20 sm:w-28 shrink-0 flex items-center justify-center"
                      style={{ background: quiz.cover_gradient || "hsl(var(--muted))" }}
                    >
                      <HelpCircle className="h-8 w-8 text-white/40" />
                    </div>
                    <div className="flex-1 p-4 min-w-0">
                      <div className="min-w-0">
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          <Badge className="shrink-0 text-xs bg-green-500/10 text-green-600 dark:text-green-400 border-green-500/20" variant="outline">
                            Published
                          </Badge>
                          {!quiz.is_public && (
                        ...
                        <div className="flex items-center gap-3 mt-2 flex-wrap">
                          <Badge variant="outline" className={`text-xs ${categoryColors[quiz.category] || ""}`}>
                            {quiz.category}
                          </Badge>
                          <Badge variant="outline" className={`text-xs capitalize ${difficultyColors[quiz.difficulty] || ""}`}>
                            {quiz.difficulty}
                          </Badge>
                          <span className="text-xs text-muted-foreground flex items-center gap-1">
                            <Play className="h-3 w-3" /> {quiz.play_count} plays
                          </span>
                          <span className="text-xs text-muted-foreground">
                            ★ {quiz.rating_count > 0 ? (quiz.rating_sum / quiz.rating_count).toFixed(1) : "—"}
                          </span>
                          <span className="text-xs text-muted-foreground">
                    ...
                    <div className="flex flex-col justify-center gap-1 p-3 border-l border-border">
                      ...
                      <Button
                        variant="ghost" size="sm" className="gap-1.5 justify-start"
                        onClick={() => setShareCode(quiz.share_code)}
                      >
                      ...
                      <Button
                        variant="ghost" size="sm" className="gap-1.5 justify-start text-destructive hover:text-destructive"
                        onClick={() => setDeleteTarget({ id: quiz.id, title: quiz.title, type: "published" })}
                      >
                        <Trash2 className="h-3.5 w-3.5" /> Delete
                      </Button>
                    </div>
      ...
      <DeleteConfirmModal
        quizTitle={deleteTarget?.title || null}
        open={!!deleteTarget}
